			...
			<form name="myform" id="myform" action="" method="post">
				<div class="result-title">
					<div class="result-list">
						...
						<a href="order.do?a=orderList"><i class="icon-font"></i>处理商品</a>
					</div>
				</div>
						...
						<c:forEach items="${sessionScope.itemList}" var="i">
							<tr>
								<td class="tc"><input name="id[]" value="59"
									type="checkbox"></td>
								<td width=20%><img src="${i.itemPic}" alt="" name="itemPic" style="width:200px;height:200px;"></td>
								<td width=50%>${i.itemDesc}</td>
								<td>${i.price}</td>
								<td>${i.count}</td>
								<td>${i.price}*${i.count}</td>
								<td>
									<a class="link-update" href="item.do?a=update&id=${i.id}">修改</a>
